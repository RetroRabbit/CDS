namespace CDS.Util.TextPrinter.Ports
{
    ...
    internal class RawPrinterHelper
    {
        ...
        public static bool SendBytesToPrinter2(string szPrinterName, byte[] bytes2, int length, string DocName)
        {
            IntPtr destination = new IntPtr(0);
            destination = Marshal.AllocCoTaskMem(length);
            Marshal.Copy(bytes2, 0, destination, length);
            bool flag = SendBytesToPrinter(szPrinterName, destination, length, DocName);
