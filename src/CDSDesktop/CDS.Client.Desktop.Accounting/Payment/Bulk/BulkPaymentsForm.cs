...
{
    public partial class BulkPaymentsForm : CDS.Client.Desktop.Accounting.Payment.Bulk.BaseBulkPaymentForm
    {
        public BulkPaymentsForm()
        {
            InitializeComponent();
        }
        ...
        {
            base.ApplySecurity();
            AllowSave = BL.ApplicationDataContext.Instance.AccessGranted(BL.SEC.AccessCodes.FIENRE08);
        }

        public override DataTable GetPaymentItems(List<Int64> accounts)
        {
            return BL.GLX.GLX_Header.GetPayable(BL.SYS.SYS_Period.GetCurrentPeriod(DataContext), String.Format("AccountId in ({0})", string.Join(",", accounts)), DataContext);
        }
